...
You can find sample outputs generated by this tool in the [`examples/`](examples/) directory of this repository. These examples include generated documentation and diagrams to help you understand what to expect from running the tool.
...
> "Please read and execute the setup instructions in [SETUP_ASSISTANT_PROMPT.md](SETUP_ASSISTANT_PROMPT.md)"
...
5. Edit [`config.json`](config.json.template) to include the repositories you want to analyze:
   ```json
   {
     "repos": [
       {
...
The tool will follow the instructions in [`CLAUDE.md`](CLAUDE.md) to analyze your configured repositories.
...
1. Read repositories from your [`config.json`](config.json.template)
2. Create or update [`state.json`](CLAUDE.md#state-management) to track progress
...
```
Then edit the file to add your GitHub token and adjust paths as needed. See [`.mcp.json.template`](.mcp.json.template) for the structure.
...
3. **Conduit MCP Tool** (Required for Confluence integration)
   - [GitHub Repository](https://github.com/codingthefuturewithai/conduit.git)
...
1. Install and configure the Conduit MCP server - see [Conduit repository](https://github.com/codingthefuturewithai/conduit.git) for installation instructions
